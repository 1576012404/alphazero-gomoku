...
class MCTS {
public:
    MCTS(shared_ptr<NeuralNetwork> neural_network, unsigned int thread_num, double c_puct,
         unsigned int num_mcts_sims, double c_virtual_loss,
         unsigned int action_size);
    std::vector<double> get_action_probs(shared_ptr<Gomoku> gomoku, double temp = 1e-3);
